...
def test_extract_chapter_思考快与慢(session_context, ebook_path: str, tmp_path: str):
    ...
    transcript_dir = os.path.join(session["process_dir"], "extracted")
    os.makedirs(transcript_dir, exist_ok=True)
    for i, c in enumerate(epub_chapter_docs):
        paragraphes = processor.extract_chapter_structured_sentence_list(c, False)
        cache_text_in_dir(str(i), paragraphes, transcript_dir)
        assert paragraphes
...
def test_filter_chapter_一句顶一万句(session_context, ebook_path: str, tmp_path: str):
    ...
    chapters_with_tn_sentences = process_chapters(processor, toc_epub_docs, transcript_dir, session)
    cache_text_in_dir(chapters_with_tn_sentences, transcript_dir)
    created_files = sum(1 for entry in os.scandir(transcript_dir) if entry.is_file())
...
def cache_text_in_dir(base_name, tuple_lines, cache_dir):
    ...
    with open(cache_path, "a", encoding="utf-8") as cache_file:
        for tuple_line in tuple_lines:
            if isinstance(tuple_line, (tuple, list)) and len(tuple_line) == 2:
                a, b = tuple_line
                cache_file.write(f"{a}: {b}\n")
            else:
                cache_file.write(f"{tuple_line}\n")
