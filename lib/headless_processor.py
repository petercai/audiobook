                    ...
                    if prepare_dirs(args["ebook"], session):
                        ...
                        if convert2epub(id, ctx):
                            epubBook = epub.read_epub(
                                session["epub_path"], {"ignore_ncx": True}
                            )
                            ...
                            metadata["title"] = metadata["title"] = (
                                metadata["title"]
                                or Path(session["ebook"]).stem.replace("_", " ")
                            )
                            metadata["creator"] = (
                                False
                            ...
                            try:
                                if len(session["metadata"]["language"]) == 2:
                                    lang_array = languages.get(
                                        part1=session["language"]
                                    )
                                    if lang_array:
                            ...
                            if session["cover"]:
                                session["toc"], session["chapters"] = get_chapters(
                                    epubBook, session
                                )
                                ...
                                if session["chapters"] is not None:
                                    if convert_chapters2audio(id, ctx):
                                        ...
                                        if exported_files is not None:
                                            chapters_dirs = [
                                                ...
                                                and os.path.isdir(
                                                    os.path.join(
                                                        session["process_dir"], dir_name
                                                    )
                                                )
                                            ]
                                            shutil.rmtree(
                                                os.path.join(
                                                    session["voice_dir"], "proc"
                                                ),
                                                ignore_errors=True,
                                            )
                                            if is_gui_process:
